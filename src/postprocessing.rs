...
impl<'a> Postprocessor<'a> {
    ...
    pub fn from_ext(ext: &str, output_dir: &Utf8Path, files_to_process: &'a [Utf8PathBuf]) -> Self {
        let lang = match ext {
    ...
    pub fn run_postprocessor(&self) -> anyhow::Result<()> {
...
impl CodegenLanguage {
    pub fn postprocessing_commands(&self) -> &[(&'static str, &[&str])] {
        match self {
